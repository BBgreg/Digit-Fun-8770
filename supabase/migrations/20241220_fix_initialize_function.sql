...
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_subscriptions') THEN
    RAISE EXCEPTION 'user_subscriptions table does not exist in public schema';
  END IF;
  
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'app_users_profile') THEN
    RAISE EXCEPTION 'app_users_profile table does not exist in public schema';
  END IF;
  
  RAISE NOTICE 'Both tables exist in public schema - function should work correctly now';
END $$;